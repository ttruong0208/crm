...
function normalizeGroupProfile(group) {
  ...
  g.aiSummary = String(g.aiSummary || "").slice(0, 600);
  g.aiIntent = String(g.aiIntent || "").slice(0, 200);
  g.aiPriority = g.aiPriority || null;
  g.aiScoredAt = g.aiScoredAt || null;
  return g;
}
...
function normalizeCrmState(state) {
  ...
  next.aiKnowledgeBase = state.aiKnowledgeBase || { documents: [] };
  return next;
}
